...
class PomdpWaypointPlanner:
    ...
    def _state_in_obstacle(self, state, obstacle):
        ...
        if state[0] <= obstacle_max_coord[0] and state[1] <= obstacle_max_coord[1] and state[0] >= obstacle_min_coord[
            0] and state[1] >= obstacle_min_coord[1]:
            return True
        else:
            return False
