...
class Environment:
	...
	def __init__(self, grid_min_coord, grid_max_coord, agent_coord, goal_coord, num_waypoints_x, num_waypoints_y):
		...
		self.num_waypoints_x = num_waypoints_x 
		self.num_waypoints_y = num_waypoints_y
		self.obstacles = []

	def add_obstacle(self, obj_bottom_left, obj_top_right):
		self.obstacles.append((obj_bottom_left, obj_top_right))

	def get_waypoint(self, x_coord, y_coord):
		return (math.floor(x_coord/self.num_waypoints_x), math.floor(y_coord/self.num_waypoints_y))
